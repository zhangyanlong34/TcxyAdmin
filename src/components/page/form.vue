    ...
    <i-form ref:form-validate :model="formValidate" :rules="ruleValidate" :label-width="80">
        ...
        <Form-item label="城市" prop="city">
            <i-select :value.sync="formValidate.city" placeholder="请选择所在地">
                <i-option  v-for="(item, index) in list" v-bind:item="item" v-bind:index="index" v-bind:key="item.value" v-bind:value="item.value">{{ item.key }}</i-option>
...
export default {
    data() {
        ...
        computed: {
           list: function () {
              return [{'key':'上海','value':'shanghai'},{'key':'北京','value':'beijing'}];
           }
        }
}
