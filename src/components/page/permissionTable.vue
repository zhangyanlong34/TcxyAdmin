<template>
  <div style="margin: 10px;overflow: hidden">
    <Modal
      ...
      v-model="del.modal"
      title="新增权限">
      <Form  ref="formUser" label-position="left" :model="permission" :rules="ruleInline"  :label-width="100">
        <FormItem label="权限名" prop="username">
          <Input type="text" v-model="permission.name"></Input>
        </FormItem>
        <FormItem label="控制器" prop="password">
          <Input type="text" v-model="permission.controller"></Input>
        </FormItem>
      ...
      <div slot="footer" align="center">
        <Button type="success" size="large"  @click="savePermission">保存</Button>
      </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      changePage (index) {
        this.$http.post(URL.permissionAllUrl,{page:(index-1),pageSize:this.pageSize}).then(function(response){
      ...
      savePermission() {
          this.$http.post(URL.permissionSaveUrl,{name:this.permission.name,controller:this.permission.controller}).then(function(response){
    ...
    mounted: function(){
      this.$nextTick(function () {
        this.$http.post(URL.permissionAllUrl,{page:0,pageSize:this.pageSize}).then(function(response){
          console.log(response.data.data);
          console.log(response.data.count);
          this.data6 = JSON.parse(response.data.data);
